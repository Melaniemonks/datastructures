...
class LinkedList:
    ...
    def remove_tail(self):
        ...
        if self.head == self.tail:
            self.head = None
            self.tail = None
            #if the list has more than one item, then just self.tail will be affected
        elif self.head is not None:
            self.tail.prev.next = None #set "next" to second to last node
            self.tail = self.tail.prev #set tail to prev to last node
    ...
    def remove(self, value):
        ...
        """
        #check is value to be removed is the head or tail
        if self.head.data == value:
            return self.remove_head()
        elif self.tail.data == value:
            return self.remove_tail()

        #set the variable to the head of the node
        curr_node = self.head
