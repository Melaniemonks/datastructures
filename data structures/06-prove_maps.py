...
class Translator:
    ...
    def translate(self, from_word):
        ...
        """
        # return value if key is in dictionary
...
def summarize_degrees(filename):
    ...
    with open(filename) as file_in:
        for line in file_in:
            ...
            # ADD YOUR CODE HERE
            # assign the 4th column of the splitted file to the variable

            the_degree = fields[3]
            if the_degree not in degrees:
                degrees[the_degree] = 1
            else: degrees[the_degree] += 1

            #return dictionaries with key and their values
    return degrees
...
def is_anagram(word1, word2):
    ...
    #ELiminate sapces and make string lowercase
    word1 = word1.replace(" ", "").lower()
    word2 = word2.replace(" ", "").lower()

    #Return false if string sizes are different
    ...
    #Create a loop to count the characters present in word1
    for i in range(len(word1)):
        word1_d[word1[i]] = word1_d.get(word1[i], 0) + 1

    #Create a loop to count the characters present in word2
    for i in range(len(word2)):
        word2_d[word2[i]] = word2_d.get(word2[i], 0) + 1
    
    
    #check if the same content is in both of the dic
    return word1_d == word2_d
...
class Maze:
    ...
    def move_left(self):
        ...
        """
        if not (self.maze_map[(self.curr_x, self.curr_y)][0]):
    ...
    def move_right(self):
        ...
        can't move, then display "Can't go that way!"
        """        
        if not (self.maze_map[(self.curr_x, self.curr_y)][1]):
            print("Can't go that way")
        else:
            self.curr_x += 1
    ...
    def move_up(self):
        ...
        can't move, then display "Can't go that way!"
        """
        if not (self.maze_map[(self.curr_x, self.curr_y)][2]):
            print("Can't go that way")
        else:
            self.curr_y -= 1
    ...
    def move_down(self):
        ...
        can't move, then display "Can't go that way!"
        """
        if not (self.maze_map[(self.curr_x, self.curr_y)][3]):
            print("Can't go that way")
        else:
            self.curr_y += 1
...
def earthquake_daily_summary():
    ...
    # ADD YOUR CODE HERE
    #loop through key "features"
    for i in data["features"]:
        earthquake = i["properties"]["type"]

    #if the value is equal to the string, print corresponding location and magnitude
